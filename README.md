...
Linquage desequilibrium (**LD**) and Principal component analysis (**PCA**) are used here to control the genetic structure of populations and avoid false positives during the GWAS. Inside the genome, SNPs can be correlated each others, consequently these SNPs would be over-represented and blow artificially the PCA. The LD allows the suppression of redundants SNPs with --indep-pairwise 50 5 0.2, which means window of 50 SNPs, shift of 5 SNPs, and remove SNPs with correlation over 0.2. The PCA is used to search for principals sources of genetic variation. The principal components of the PCA will be used in the GWAS model as covariables to compare similar individuals genetically. The variants were named before LD pruning and PCA [Variants_named_bwt05_06.sh]. The LD pruning and PCA were performed on the genomic data after filtering for all individuals and only males [SEE 05.LD_pruning_pca-slurm & 06.plot_PCA_baseR.R].
...
The PCA confirms a strong population structure with and without females between India and Myanmar by separated clusters. The first and second principal component explain 7.70% and 6.06%. These distinct clusters group together closed individuals genetically, meaning that individuals from one population are genetically closer from the other population. The PCA by tusk displays a real mixed pattern between tuskless and tusked individuals. Consequently, phenotype tusk is not simply equivalent to a population or a PCA cluster. For GWAS models, it is important to include PCs as covariables 
