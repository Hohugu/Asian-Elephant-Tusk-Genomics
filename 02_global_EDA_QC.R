...
fig_dir  <- file.path(base_dir, "PreGWAS/figures")
tab_dir  <- file.path(base_dir, "PreGWAS/tables")

dir.create(fig_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(tab_dir, showWarnings = FALSE, recursive = TRUE)

prefix <- file.path(res_dir, "GWAS_raw")
...
het   <- read.table(
  paste0(prefix, ".heterozygosity.het"),
  header = TRUE,
  fill = TRUE,
  stringsAsFactors = FALSE
)
...
summary_pheno <- as.data.frame(table(pheno$tusk, pheno$sex, pheno$population))
names(summary_pheno) <- c("tusk", "sex", "population", "N")
...
write.table(summary_pheno, file.path(tab_dir, "phenotype_summary.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(sample_qc[order(sample_qc$F_MISS, decreasing = TRUE), ],
            file.path(tab_dir, "sample_missingness_with_pheno.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(summary_variants, file.path(tab_dir, "variant_global_summary.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(snps_per_contig, file.path(tab_dir, "snps_per_contig.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(het_qc[order(het_qc$F), ],
            file.path(tab_dir, "heterozygosity_with_pheno.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
...
png(file.path(fig_dir, "09_heterozygosity_by_sex.png"), width = 900, height = 700)
boxplot(F ~ sex,
        data = het_qc,
        main = "Heterozygosity by sex",
        xlab = "Sex",
        ylab = "F")
dev.off()
